...
def day_time(day_plan:dict):
    st_time = 60*int(day_plan["activities"][0]["start_time"].split(":")[0])+int(day_plan["activities"][0]["start_time"].split(":")[1])
    ed_time = 60*int(day_plan["activities"][-1]["end_time"].split(":")[0])+int(day_plan["activities"][-1]["end_time"].split(":")[1])
...
def plan_to_poi(plan: dict):
    ...
    target_city_start_station_lon_lat = get_lon_lat(city=city,poi_name=target_city_start_station)
    target_city_end_station_lon_lat = get_lon_lat(city=city,poi_name=target_city_end_station)
    
    new_itinerary = []
    for day_plan in plan["itinerary"]:
        new_day_plan = {"day": day_plan["day"], "position": [],"cost":0,"time":"0h 0min"}
        plan_day_poi = []
        poi_detail = []
        for activity in day_plan["activities"]:
...
def get_trans_time_total(trans:list):
    total_time = 0
    for t in trans:
        st_time = 60*int(t["start_time"].split(":")[0])+int(t["start_time"].split(":")[1])
        ed_time = 60*int(t["end_time"].split(":")[0])+int(t["end_time"].split(":")[1])
        total_time += ed_time-st_time
    return total_time
        ...
        for activity in day_plan["activities"]:
            ...
            if len(activity["transports"]) != 0:
                ...
                for i in range(len(act["trans_detail"])):
                    act["trans_detail"][i]["distance"] = round(act["trans_detail"][i]["distance"],2)
            act["position"] = activity["position"] if "position" in activity else activity["start"]
            act["type"] = activity["type"]
            if act["type"] in ["breakfast", "lunch", "dinner"]:
                food_list = restaurants_tool.select(city=city,key="name",func= lambda x:x == act["position"])
            ...
            act["picture"] = get_image_url(city=city,poi_type=act["type"],name=act["position"])
...
def main(request: dict, task_id,debug_mode=False):
    ...
    nature_language = f"当前位置{start_city},打算{people_number}个人一起去{target_city}旅游{days}天,"
    if addition_info != "":
    ...
    query["hard_logic"] = [logic_str for logic_str in query["hard_logic"] if not (logic_str.startswith("days") or logic_str.startswith("people_number"))]
