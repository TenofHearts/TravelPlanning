...
def search_keywords(keywords, region=None,show_fields="business,indoor,photos"):
    ...
    try:
        ...
        print("解析返回的JSON数据失败")
        return None
def search_routine(origin, destination, city1, city2, type, time=None, originpoi=None, destinationpoi=None):
    """调用高德API进行公交路线规划
    ...
    try:
        ...
        if raw_data and 'route' in raw_data and 'transits' in raw_data['route'] and len(raw_data['route']['transits']) > 0:
            # 取第一个方案的时间信息
            ...
            if 'cost' in first_route:
                ...
                if time:
                    try:
                        ...
                        # 计算总时间分钟数
                        total_minutes = start_hour * 60 + start_minute + (duration_seconds // 60)
            ...
            if 'bus' in segment and 'buslines' in segment['bus']:
                ...
                for busline in buslines:
                    bus_info = {
                        ...
                        'departure_stop': busline.get('departure_stop', {}).get('name', '未知站点'),
                        'arrival_stop': busline.get('arrival_stop', {}).get('name', '未知站点'),
...
def test_api_routine_with_real_api():
    ...
    if result:
        print("\n=== 原始数据 ===")
        print(json.dumps(result['raw_data'], indent=2, ensure_ascii=False)[:500] + "..." if len(json.dumps(result['raw_data'], indent=2, ensure_ascii=False)) > 500 else json.dumps(result['raw_data'], indent=2, ensure_ascii=False))
