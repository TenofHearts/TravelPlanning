...
SUBWAY_PATH = os.path.join(curdir, "../../database/transportation/subways.json")
city_list = [
    "shanghai",
    "beijing",
    "shenzhen",
...
def GoTo(city, locationA, locationB, start_time, transport_type, verbose=True):
    ...
    print("Line94",city, locationA, locationB, start_time, transport_type, verbose)
    ...
    routine = search_routine(
        coordinate_A, coordinate_B, citycode_A, citycode_B, "0", start_time_in
    )
    if transport_type == "walk":
        ...
        if routine["time_cost"]=="" or routine["time_cost"].get("end_time", "") == "":  # 说明距离太近，直接走路过去
