                ...
                if enable_detailed_search:
                    try:
                        select_restaurant = restaurants.select(
                            target_city,
                            key="name",
                            func=lambda x: x == activity.get("position", ""),
                        )
                        if (
                            not select_restaurant.empty
                            and "cuisine" in select_restaurant.columns
                        ):
            ...
            if activity_type == "accommodation":
                ...
                if enable_detailed_search:
                    try:
                        ...
                        if (
                            not select_hotel.empty
                            and "featurehoteltype" in select_hotel.columns
                        ):
            ...
            if activity_type == "attraction":
                ...
                if enable_detailed_search:
                    try:
                        ...
                        if (
                            not select_attraction.empty
                            and "type" in select_attraction.columns
                        ):
...
def evaluate_hard_constraints(symbolic_input_list, plan_json_list):
    ...
    for symbolic_input, plan_json in zip(symbolic_input_list, plan_json_list):
        try:
            ...
            extracted_vars = None
        results.append(
            evaluate_constraints(extracted_vars, symbolic_input["hard_logic"])
        )
