...
def reason_prompt(
    info_list, return_history_message=True, history_message=[], verbose=False
):
...
def get_poi_type_from_time(current_time, candidates_type, back_transport_time):
    ...
    if "back-intercity-transport" in candidates_type:
        info_list.append(
            "back-intercity-transport: 今天是旅程的最后一天，可以选择回家,返程车票时间是{}，请预留时间用于交通到达相应地点".format(
                back_transport_time
            )
        )
    ...
    info_list.append(
        "当前可选择的候选类型包括: {}, 作为一个旅行规划助手，请结合以上信息进行思考".format(
            candidates_type
        )
    )
    ...
    if interactive_mode == "human":
        ...
        if poi_type in candidates_type:
            ...
            while 3 < 33:
                ...
                print(
                    "请在候选类型中做出选择，保证输入类型是当前可选择的候选类型包括: {}".format(
                        candidates_type
                    )
                )
                poi_type = input()
        ...
        result = react_prompt(
            info_list, action_prompt, return_history_message=False, history_message=[]
        )
...
def score_poi_think_overall_act_page(
    planning_info, poi_info, react=False, history_message=[]
):
...
def recommend_poi_time(planning_info, poi_name):
    ...
    info_list.append(
        "请你根据以上信息，思考我在{}的游玩时间长度,请注意你只需要思考我在这个景点的游玩即可，不用给出我完整的行程".format(
            poi_name
        )
    )
...
def llm_next_page(hisorty_message, previous_score, current_page):
    ...
    info_list = [
        "你已经查了{}个信息，对它们的打分是:{}, 如果以上候选都不满足要求，那么你需要进一步查询更多信息,请思考是否满足了对旅行规规划的要求".format(
    ...
    result, history_message = react_prompt(
        ...
        action_prompt,
        return_history_message=True,
        history_message=hisorty_message,
    )
...
def search_poi(
    query, poi_plan, plan, current_time, current_position, current_day=0, verbose=False
):
    ...
    while len(candidates_type) > 0:
        ...
        if poi_type == "back-intercity-transport":
            ...
            info_list.append(
                "在这次在{}的旅行中我们之前去过了{}，不要重复去".format(
                    query["target_city"], str(restaurant_names_visiting)
                )
            )
            info_list.append(
                "现在是第{}天 {}, 请帮我选择一个餐厅去吃{}".format(
            ...
            if "cost_wo_intercity" in query:
                ...
                info_list.append(
                    "本次旅行有整体开销的约束，剩余用于餐饮和景点的预算为: {} 元".format(
                        rest_cost
                    )
                )
            ...
            if not fast_mode:
                ...
                if "food_type" in query and (
                    not set(query["food_type"]) <= set(food_type_visiting)
                ):
                ...
                # print(res_weight)
                if "restaurant_names" in query and (
                    not set(query["restaurant_names"]) <= set(restaurant_names_visiting)
                ):

                    req_res_type = set(restaurant_names_visiting) - set(
                        query["restaurant_names"]
                    )
                ...
                while current_page * item_per_page - item_per_page < len(rest_info):
                    ...
                    for idx in range(len(current_info)):
                        ...
                        poi_info_list.append(
                            "**{}** name: {} , arrived_time: {}, cusine: {}, price_per_preson: {}".format(
                                idx,
                                res_i["name"],
                                arrived_time,
                                res_i["cuisine"],
            ...
            for r_i in ranking_idx:
                ...
                if not (res_idx in restaurants_visiting):
                    ...
                    # 开放时间
                    opentime, endtime = (
                        poi_sel["weekdayopentime"],
                        poi_sel["weekdayclosetime"],
                    )
            ...
            if not fast_mode:
                ...
                if (
                    "spot_type" in query
                    and (not set(query["spot_type"]) <= set(spot_type_visiting))
                ) or ("spot_type_unseen" in query):
                    ...
                    if "spot_type" in query:
                        req_spot_type = set.union(
                            req_spot_type,
                            set(query["spot_type"]) - set(spot_type_visiting),
                        )
                ...
                if (
                    "attraction_names" in query
                    and (
                        not set(query["attraction_names"])
                        <= set(attraction_names_visiting)
                    )
                ) or ("attraction_names_unseen" in query):

                    req_attr_name = set()
                    if "attraction_names" in query:
                        req_attr_name = set.union(
                            req_attr_name,
                            set(query["attraction_names"])
                            - set(attraction_names_visiting),
                        )

                    if "attraction_names_unseen" in query:
                        req_attr_name = set.union(
                            req_attr_name, set(query["attraction_names_unseen"])
                        )
                ...
                while current_page * item_per_page - item_per_page < len(
                    poi_info["attractions"]
                ):
                    ...
                    page_l = current_page * item_per_page - item_per_page
                    page_r = min(
                        current_page * item_per_page, len(poi_info["attractions"])
                    )
                    ...
                    for idx in range(len(current_info)):
                        ...
                        # poi_info_list.append("**{}** name: {} , arrived_time: {}, cusine: {}, price_per_preson: {}, recommended food: {}".format(idx, res_i["name"], arrived_time, res_i["cuisine"], res_i["price"], res_i["recommendedfood"]))
                        poi_info_list.append(
                            "**{}** name: {} , arrived_time: {}, price_per_preson: {}".format(
...
def search_plan(query, poi_plan, fast_mode=False, result_dir=None):
    ...
    print(
        "{} accommmodation, {} hotels (satisfied requirments)".format(
    ...
    if fast_mode:
        ...
        try:
            ...
            if "cost_wo_intercity" in query:
                ...
                info_list.append(
                    "本次旅行有整体开销的约束，所有餐饮和景点的预算为: {} 元".format(
                        rest_cost
                    )
                )
                upper_budget = rest_cost / query["people_number"]
    ...
    for go_i in range(train_go_num + flight_go_num):
        ...
        for back_i in range(flight_back_num + train_back_num - 1, -1, -1):
            ...
            if back_i >= flight_back_num:
                ...
                if (
                    "train_type" in query
                    and query["train_type"] != poi_plan["back_transport"]["TrainID"][0]
                ):
                    continue
            else:
            ...
            if query["days"] > 1:
                ...
                for hotel_i in range(num_hotel):
                    ...
                    if ("hotel_price" in query) and int(
                        hotel_info.iloc[hotel_i]["price"]
                    ) > query["hotel_price"]:
                        continue

                    if "cost" in query:
                        hotel_cost = (
                            int(hotel_info.iloc[hotel_i]["price"])
                            * query["people_number"]
                            * (query["days"] - 1)
                        )
...
if __name__ == "__main__":
    ...
    for idx in test_idx:
        ...
        if not success:
            ...
            with open(
                result_dir + "/fail_list.txt".format(query_level), "a+"
            ) as dump_f:
            ...
            with open(
                result_dir + "/plan_{}.json".format(idx), "w", encoding="utf8"
            ) as dump_f:
